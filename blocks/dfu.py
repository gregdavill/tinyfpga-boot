...
class DFUHandler(USBRequestHandler):
    def __init__(self, if_num, areas, runtime=False):
        super().__init__()
        ...
        self.state = Signal(8, reset = DFUState.appIDLE if runtime else DFUState.dfuIDLE)
        self.runtime = runtime
        self.last_addr = Signal(24)
    ...
    def handle_dnload(self, m):
        ...
        with m.If(self.new_request):
            ...
            m.d.usb += self.last_addr.eq(self.last_addr + self.interface.setup.length - 1)

            # Starting a new DNLOAD cycle?
            with m.If(self.state == DFUState.dfuIDLE):
                m.d.usb += [
                    self.addr.eq(self.areas[self.area_sel]),
                    self.last_addr.eq(self.areas[self.area_sel]),
    ...
    def elaborate(self, platform):
        ...
        m.submodules.fifo = self.fifo = TransactionalizedFIFO(width = 8, depth = 512, domain = 'usb')
        ...
        m.d.comb += [
            ...
            self.source.last.eq(self.addr == self.last_addr),
        ]
